...
class AwsDbCloudwatchAlarmsCoverage < AwsResourceBase
  include AwsDbComplianceShared

  name "aws_db_cloudwatch_alarms_coverage"
  desc "CloudWatch alarms coverage for database services (CIS §4.7 / §5.6 / §6.6 / §7.8 / §9.7 / §10.8)."
  example "
    inv = aws_db_cloudwatch_alarms_coverage(namespaces: ['AWS/RDS', 'AWS/DocDB'])
    describe inv do
    ...
    loop do
      resp =
        begin
          ...
          record_access_denied_or_warn("aws_db_cloudwatch_alarms_coverage", region, "describe_alarms", e)
          return alarms
        end
